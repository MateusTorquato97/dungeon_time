...
class CombatService {
    ...
    async processarAcaoTurno(batalhaId, turno, acao) {
        ...
        try {
            ...
            // Processa efeitos decorrentes do turno
            const participante = await this.getParticipanteTurno(client, batalhaId, acao.usuarioId);
            const participanteTurno = participante;
            await this.processarEfeitos(client, batalhaId, participanteTurno.id);

            // Insere um log inicial para a ação e retorna o id
            const logResult = await client.query(
                `INSERT INTO logs_batalha 
                 (batalha_id, turno, tipo_acao, usuario_origem, usuario_alvo, habilidade_id, dano, cura, efeitos)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                 RETURNING id`,
                [
                    batalhaId, turno, acao.tipo, acao.usuarioId, acao.alvoId,
                    acao.habilidadeId, 0, 0, JSON.stringify({})
                ]
            );
            const logId = logResult.rows[0].id;

            // Se for uma habilidade, processa ela primeiro para aplicar os efeitos imediatamente
            if (acao.tipo === 'habilidade') {
                const resultado = await this.processarHabilidade(client, participante, acao);

                // Atualiza o log com os resultados da habilidade
                await client.query(
                    `UPDATE logs_batalha 
                     SET dano = $1, cura = $2, efeitos = $3
                     WHERE id = $4`,
                    [resultado.dano, resultado.cura, JSON.stringify(resultado.efeitos), logId]
                );

                // Se houver dano ou cura, atualiza o status do alvo
                if (resultado.dano > 0 || resultado.cura > 0) {
                    await this.atualizarStatusParticipante(client, batalhaId, acao.alvoId, resultado.dano, resultado.cura);
                }
            ...
            // Processa a ação de ataque depois que os efeitos já foram aplicados
            if (acao.tipo === 'ataque') {
                await this.processarAcao(client, batalhaId, turno, acao);
            }
    ...
    async processarAcao(client, batalhaId, turno, acao) {
        ...
        if (acao.tipo === 'habilidade') {
            ...
            // Só aplica o dano se o ataque não foi esquivado
            if (resultado.dano > 0) {
                dano = resultado.dano;
        ...
        // Atualiza o log inserido com os dados da ação
        await client.query(
            `UPDATE logs_batalha 
             SET dano = $1, cura = $2, efeitos = $3
             WHERE batalha_id = $4 AND turno = $5 AND usuario_origem = $6
             AND id = (SELECT id FROM logs_batalha WHERE batalha_id = $4 AND turno = $5 AND usuario_origem = $6 ORDER BY id ASC LIMIT 1)`,
            [dano, cura, JSON.stringify(efeitos), batalhaId, turno, acao.usuarioId]
        );

        // Só atualiza o status do alvo se houver dano ou cura para aplicar
        if (dano > 0 || cura > 0) {
            await this.atualizarStatusParticipante(client, batalhaId, acao.alvoId, dano, cura);
        }
    }
    ...
    async calcularDanoBasico(client, batalhaId, atacante, acao) {
        ...
        if (bloqueio > 0) {
            // Se houver bloqueio, reduz o dano final baseado no valor do bloqueio
            // O bloqueio é uma porcentagem, então 100 significa bloqueio total
            const reducaoBloqueio = Math.min(100, bloqueio) / 100;
            danoFinal = Math.floor(danoFinal * (1 - reducaoBloqueio));
            return { dano: danoFinal, critico };
        }
    ...
    async processarHabilidade(client, participante, acao) {
        ...
        const habilidade = efeitosRows[0];
        let resultado = {
            dano: 0,
            cura: 0,
            efeitos: {}
        ...
        for (const efeito of efeitosRows) {
            if (Math.random() <= efeito.chance / 100) {
                switch (efeito.tipo_efeito) {
                    case 'dano':
                        resultado.dano += await this.calcularDanoHabilidade(client, participante, efeito);
                        break;
                    case 'cura':
                        resultado.cura += await this.calcularCura(client, participante, efeito);
                        break;
                    case 'bloqueio':
                        // Para efeitos de bloqueio, inserimos na tabela efeitos_batalha
    ...
    async calcularDanoHabilidade(client, participante, efeito) {
        const personagemResult = await client.query(
            `SELECT p.*, pe.arma_id 
             FROM personagens p 
             LEFT JOIN personagem_equipamentos pe ON p.usuario_id = pe.usuario_id 
    ...
    async calcularCura(client, participante, efeito) {
        ...
        const personagem = personagemResult.rows[0];
        let curaBase = efeito.valor;
        if (efeito.atributo_base) {
            curaBase = Math.floor(curaBase * (personagem[efeito.atributo_base] / 100));
        }
        return curaBase;
    }
    ...
    async verificarBloqueio(client, participanteId) {
        console.log('Verificando bloqueio para participante ID:', participanteId);
        const result = await client.query(
            `SELECT * 
        ...
        console.log('Resultado da consulta de bloqueio:', result.rows);

        if (result.rows.length > 0) {
            console.log('Bloqueio encontrado:', result.rows[0]);
    ...
    async getBatalhaStatus(clientOrBatalhaId, maybeBatalhaId) {
        let client, batalhaId;
        if (typeof clientOrBatalhaId === 'object' && maybeBatalhaId) {
            client = clientOrBatalhaId;
            batalhaId = maybeBatalhaId;
        } else {
            batalhaId = clientOrBatalhaId;
            ...
            for (let i = 0; i < logs.length; i++) {
                ...
                if (log.tipo_acao === 'ataque') {
                    ...
                    if (log.dano > 0) {
                        ...
                        text += `${log.origem_nickname} usou a habilidade ${log.nome_habilidade} em ${log.alvo_nickname}${criticoText}, causando ${log.dano} de dano. ${log.alvo_nickname} ficou com ${log.vida_alvo_pos_acao} de vida.`;
                    } else {
